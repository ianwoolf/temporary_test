 ...
 * OpenAI API provider
 */
class OpenAiProvider(private val config: AiProviderConfig) : AiService {

    companion object {
        private const val DEFAULT_BASE_URL = "https://api.openai.com/v1"
        private const val DEFAULT_MODEL = "gpt-4"
    ...
    override suspend fun generateTestFlow(documentation: String): Result<TestFlow> = withContext(Dispatchers.IO) {
        try {
            if (!isConfigured()) {
                return@withContext Result.failure(AiServiceException("OpenAI API key is not configured"))
            }
            ...
            val requestBody = mapOf(
                "model" to model,
                "messages" to listOf(
                    mapOf("role" to "system", "content" to PromptBuilder.buildSystemPrompt()),
            ...
            if (!response.isSuccessful) {
                ...
                return@withContext Result.failure(
                    AiServiceException("OpenAI API error: ${response.code}, $errorBody")
                )
            }

            val responseBody = response.body?.string()
                ?: return@withContext Result.failure(AiServiceException("Empty response from OpenAI"))
